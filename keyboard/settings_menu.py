...
def build_settings_keyboard(is_vertical: bool, duration_sec: int, size: str) -> InlineKeyboardMarkup:
    ...
    row3 = [
        InlineKeyboardButton(text=f"{_checkmark(sz_small)}Низкое качество", callback_data="set:size:small"),
        InlineKeyboardButton(text=f"{_checkmark(sz_large)}Высокое качество", callback_data="set:size:large"),
    ]
